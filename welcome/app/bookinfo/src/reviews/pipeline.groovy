pipeline {
    ...
    parameters {
       ...
       string( name: 'imageName' , defaultValue : "reviews")  // Replace with your Docker image name
        string (name : 'BUILD_NUMBER', defaultValue : "${env.BUILD_NUMBER}")
        string (name : 'token', defaultValue : "123")
        string(name : 'branch', defaultValue : "jenkins-workshop")
        string(name : 'dockerfile', defaultValue : "devopshift-welcome/welcome/app/bookinfo/src/reviews" ) 
    }
    stages {
        stage('Checkout') {
            steps {
        ...
        stage('security') {
            steps {
                script {
                // Run sectests inside the Docker container
                sh "docker build -f ${dockerfile}/Dockerfile --build-arg SNYK_TOKEN=${token} -t shivi2021/${imageName}:1.0.${BUILD_NUMBER}-security ${dockerfile}"
                sh "docker run -it -d -p 9080:9080 --name ${imageName}-sec shivi2021/${imageName}:1.0.${BUILD_NUMBER}-security" // docker run -it -d -p 9080:9080 --name productpage shivi2021/productpage:1.0.5
                sh "docker ps"
                sh "docker stop ${imageName}-sec"
                sh "docker rm ${imageName}-sec"
                }
            }
        }
